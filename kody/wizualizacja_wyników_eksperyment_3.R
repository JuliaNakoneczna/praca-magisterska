...
###### odporność klasyfikatorów

noise_levels <- c(0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5)

banknote_eksp3_resistance <- lapply(noise_levels, function(level){
  results <- lapply(classifiers_ALL, function(clasif){
    experiment_3_resistance(data = banknote_standarize, data_name = "banknote_standarize", label = "Status", 
                      classifier = clasif$classifier, noise_level = level, classifier_par = clasif$par, classifier_name = clasif$name)})
  list(noise_level = level, results = results)})
save(banknote_eksp3_resistance, file = file.path("praca_magisterska_wyniki_eksperyment3/zbiorcze", "banknote_resistance.RData"))

spam_eksp3_resistance <- lapply(noise_levels, function(level){
  results <- lapply(classifiers_ALL, function(clasif){
    experiment_3_resistance(data = spam_standarize, data_name = "spam_standarize", label = "type", 
                            classifier = clasif$classifier, noise_level = level, classifier_par = clasif$par, classifier_name = clasif$name)})
  list(noise_level = level, results = results)})
save(spam_eksp3_resistance, file = file.path("praca_magisterska_wyniki_eksperyment3/zbiorcze", "spam_resistance.RData"))

vehicle_eksp3_resistance <- lapply(noise_levels, function(level){
  results <- lapply(classifiers_ALL, function(clasif){
    experiment_3_resistance(data = vehicle_standarize, data_name = "vehicle_standarize", label = "Class", 
                            classifier = clasif$classifier, noise_level = level, classifier_par = clasif$par, classifier_name = clasif$name)})
  list(noise_level = level, results = results)})
save(vehicle_eksp3_resistance, file = file.path("praca_magisterska_wyniki_eksperyment3/zbiorcze", "vehicle_resistance.RData"))

classifier_resistance <- function(data, name, metrics = c("Accuracy", "Precision", "Recall", "F1_score")){
  
  results <- bind_rows(lapply(data, function(x){bind_rows(x$results)}))
  
  results <- results %>%
    group_by(Classifier, Noise_level) %>%
    summarise(across(all_of(metrics), mean, na.rm = TRUE),
              .groups = "drop") %>%
    pivot_longer(cols = all_of(metrics), names_to = "Metric", values_to = "Value") %>%
  ...
  plot <- ggplot(results, aes(x = Noise_level, y = Value, color = Classifier)) +
    geom_line(linewidth = 0.75) + 
    geom_point(size = 1.5) +
    facet_grid(Metric ~.) +
    coord_cartesian(ylim = c(0.4, 1)) +
    scale_color_manual(name = "Klasyfikator:", values = c("Drzewo decyzyjne" = "#008080", "5-NN" = "#FF69B4", "Regresja logistyczna" = "#000080")) +
...
classifier_resistance(
  banknote_eksp3_resistance,
  "D:/2 stopień/4 semestr/praca dyplomowa/kody/eksperyment_3/banknote_eksp3_resistance.pdf")

classifier_resistance(
  spam_eksp3_resistance,
  "D:/2 stopień/4 semestr/praca dyplomowa/kody/eksperyment_3/spam_eksp3_resistance.pdf")

classifier_resistance(
  vehicle_eksp3_resistance,
  "D:/2 stopień/4 semestr/praca dyplomowa/kody/eksperyment_3/vehicle_eksp3_resistance.pdf")