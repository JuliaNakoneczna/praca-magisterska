...
experiment_3 <- function(data, data_name, label, algorithm, algorithm_name, algorithm_par = list(), noise_level = 0.2, noise_type = "NCAR", 
  ...
  for (i in 1:iterations){
    ...
    if(data_name == "spam_standarize"&& algorithm_name == "EF"){
      data_spam <- subset(train_data, subset=type=="spam", select=-type)
      data_nonspam <- subset(train_data, subset=type=="nonspam", select=-type)
      sd.spam   <- apply(data_spam, 2, sd)
      sd.nospam <- apply(data_nonspam, 2, sd)
      sd.zero.id <- which(sd.spam==0 | sd.nospam==0)
      if(length(sd.zero.id) > 0){ 
        train_data <- train_data[, -sd.zero.id]}}
    ...
    if(data_name == "spam_standarize" && algorithm_name == "EF"){
      clean_data_spam <- subset(clean_train_data, subset=type=="spam", select=-type)
      clean_data_nonspam <- subset(clean_train_data, subset=type=="nonspam", select=-type)
      sd.spam   <- apply(clean_data_spam, 2, sd)
      sd.nospam <- apply(clean_data_nonspam, 2, sd)
      sd.zero.id <- which(sd.spam==0 | sd.nospam==0)
      if(length(sd.zero.id) > 0){ 
        clean_train_data <- clean_train_data[, -sd.zero.id]}}
...
experiment_3_resistance <- function(data, data_name, label, noise_level = 0.2, noise_type = "NCAR", 
                              classifier, classifier_par = list(), classifier_name, iterations = 20, seed = 12345, save_dir = "praca_magisterska_wyniki_eksperyment3"){
  result_list <- list()
  ...
  for (i in 1:iterations){
    ...
    metrics <- bind_rows(metrics_noise_data)
    metrics$Data <- data_name
    metrics$Classifier <- classifier_name
    metrics$Noise_level <- noise_level
    metrics$Noise_type <- noise_type
    ...
    result_list[[i]] <- metrics
    result_list_ALL <- bind_rows(result_list)
    name <- paste(data_name, classifier_name, noise_level, noise_type, sep = "_")
    assign(name, result_list_ALL)
    save_path <- file.path(save_dir, paste0(name, ".RData"))  
    save(list = name, file = save_path)}
